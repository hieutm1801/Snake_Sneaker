...
public class RRPolicy extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        method = new Method(RRPolicy.this);
        method.forceRTLIfSupported();

        toolbar = findViewById(R.id.toolbar_privacy_policy);
        toolbar.setTitle(getResources().getString(R.string.refund_return_policy));
    ...
    public void rr_policy() {

        progressBar.setVisibility(View.VISIBLE);

        JsonObject jsObj = (JsonObject) new Gson().toJsonTree(new API(RRPolicy.this));
        jsObj.addProperty("type", "refund_policy");
        ApiInterface apiService = ApiClient.getClient().create(ApiInterface.class);
        Call<RRPolicyRP> call = apiService.getRRPolicy(API.toBase64(jsObj.toString()));
        call.enqueue(new Callback<RRPolicyRP>() {
            @SuppressLint("SetJavaScriptEnabled")
            @Override
            public void onResponse(@NotNull Call<RRPolicyRP> call, @NotNull Response<RRPolicyRP> response) {

                try {

                    RRPolicyRP RRPolicyRP = response.body();
                    assert RRPolicyRP != null;

                    if (RRPolicyRP.getStatus().equals("1")) {
                        ...
                        method.alertBox(RRPolicyRP.getMessage());
                    }
            ...
            @Override
            public void onFailure(@NotNull Call<RRPolicyRP> call, @NotNull Throwable t) {
